    ...
    std::fs::read_dir(dir)
        ...
        .filter(|e| {
            e.metadata()
                ...
                .is_some_and(|t| t.elapsed().unwrap_or_default().as_secs() < 20)
        })
...
impl WatcherRegexes {
    fn join() -> &'static Regex {
        ...
        RE.get_or_init(|| Regex::new(r"! Joining game '([0-9a-f\-]+)' place (\d+)").unwrap())
    }

    fn joined() -> &'static Regex {
        static RE: OnceLock<Regex> = OnceLock::new();
        RE.get_or_init(|| Regex::new(r"serverId: ([0-9\.]+)\|").unwrap())
    }

    fn leave() -> &'static Regex {
        static RE: OnceLock<Regex> = OnceLock::new();
        RE.get_or_init(|| Regex::new(r"Time to disconnect replication data").unwrap())
    }
...
async fn handle_joined_event(
    ...
    let is_debounced = state
        .last_rpc
        .is_some_and(|last| now.duration_since(last).as_secs() <= 2);
