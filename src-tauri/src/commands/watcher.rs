...
async fn run_watcher(app: AppHandle) -> Result<(), String> {
    ...
    let store = Arc::new(app.store("config.json").map_err(|e| e.to_string())?);
...
async fn process_line(
    ...
    if let Some(caps) = re_udmux.captures(line) {
        ...
        handle_udmux_ip(app, ip, &store).await?;
    }

    if re_joined.is_match(line) {
        handle_joined_event(app, activity, last_rpc, &store).await?;
    }

    if re_leave.is_match(line) {
        handle_leave_event(app, activity).await?;
    }
...
async fn handle_joined_event(
    ...
    if let Some(name) = fetch_place_name(place_id).await? {
        let state = app.state::<RpcState>();
        if let Err(e) = apply_rpc(&state.client, "Playing Roblox", &name).await {
...
async fn handle_leave_event(app: &AppHandle, activity: &mut Activity) -> Result<(), String> {
    if !activity.in_game {
        return Ok(());
    }
