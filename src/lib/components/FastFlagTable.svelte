...
<div class="flex flex-col gap-6 w-full">
    <!-- Add row -->
    <div
        class="flex items-center gap-3 p-4 bg-[#0f0f0f] rounded-2xl border border-stone-800/40 shadow-sm"
    >
        <div class="flex-[2]">
            <input
                type="text"
    ...
    <div class="flex flex-col gap-4">
        <div class="relative group">
            ...
            >
                <Search
                    class="h-5 w-5 text-stone-600 group-focus-within:text-blue-500 transition-colors"
                />
            </div>
            <input
                type="text"
        ...
        <div
            class="flex flex-col rounded-2xl border border-stone-800/40 bg-[#0f0f0f] overflow-hidden"
        >
            <div
                class="flex items-center px-6 py-3 text-[11px] font-bold uppercase tracking-[0.1em] text-stone-500 border-b border-stone-800/40 bg-stone-900/20"
            >
            ...
            <div class="flex flex-col divide-y divide-stone-800/30">
                ...
                {:else}
                    {#each filteredFlags as [name, value] (name)}
                        ...
                        <div
                            class="group flex items-center px-6 py-2.5 hover:bg-stone-800/10 transition-colors duration-150"
                        >
                            <div
                            ...
                            <div class="flex-[1] flex items-center">
                                ...
                                {:else}
                                    <input
                                        ...
                                        class="w-full bg-stone-900/0 hover:bg-stone-900/40 border border-transparent hover:border-stone-800/50 focus:bg-stone-900/60 focus:border-blue-500/40 rounded-lg px-3 py-1.5 font-mono text-[13px] {type ===
                                        'int'
                                            ? 'text-yellow-400'
                                            : 'text-green-400'} outline-none transition-all"
                                    />
                                {/if}
                            </div>
