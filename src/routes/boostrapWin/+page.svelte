<script lang="ts">
    ...
    const unsub = themeStore.subscribe(v => { state = v })
    onDestroy(unsub)
    ...
    onMount(async () => {
        ...
        if (state) {
            ...
            // Handle TitleBar
            const titleBar = state.config.elements.find(e => e.type === 'TitleBar')
    ...
    function marginStyle(m?: { top:number; right:number; bottom:number; left:number }) {
        if (!m) return ''
        return [
                    ...
                    {#if downloadTotal > 0}
                        <div class="flex flex-col gap-1.5">
                            ...
                            <div class="w-full h-1 bg-stone-800 rounded-full overflow-hidden">
                                <div class="h-full bg-stone-200 rounded-full transition-all duration-300"
                                    style="width: {(downloadDone / downloadTotal) * 100}%"></div>
                            </div>
                        </div>
                    {/if}
                    {#if extractTotal > 0}
                        <div class="flex flex-col gap-1.5">
                            ...
                            <div class="w-full h-1 bg-stone-800 rounded-full overflow-hidden">
                                <div class="h-full bg-stone-200 rounded-full transition-all duration-300"
                                    style="width: {(extractDone / extractTotal) * 100}%"></div>
                            </div>
                        </div>
                    {/if}
                </div>
...
<style>
    @keyframes indeterminate {
        ...
        50%  { transform: translateX(80%)   scaleX(1.6); }
