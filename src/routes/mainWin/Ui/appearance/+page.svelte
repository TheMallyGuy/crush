...
<div class="flex flex-col gap-8 max-w-2xl">
  <div>
    <h1 class="text-3xl font-bold tracking-tight text-stone-100">Appearance</h1>
    <p class="text-stone-400 mt-1">Customize the look and feel of the Roblox bootstrapper.</p>
  </div>
    ...
    >
      ...
      <div slot="footer" class="flex flex-col w-full gap-4">
        ...
        {#if themeType === 'custom' && activeName}
          <div class="flex items-center gap-2 px-3 py-2 bg-stone-900/50 border border-stone-800 rounded-lg">
            <svg class="w-4 h-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="text-sm text-stone-300">Active: <span class="text-stone-100 font-medium">{activeName}</span></span>
          </div>
        {/if}

        {#if missing.length > 0}
          <div class="bg-yellow-950/30 border border-yellow-900/50 rounded-xl px-4 py-3 text-yellow-500 text-sm">
            ...
            <ul class="list-disc list-inside text-xs opacity-80 space-y-0.5 ml-1">
              {#each missing as f}
        ...
        {#if error}
          <div class="bg-red-950/30 border border-red-900/50 rounded-xl px-4 py-3 text-red-400 text-sm flex items-center gap-2">
