<script lang="ts">
    ...
    async function loadConfig() {
        ...
        const savedIntegrations = ((await store.get<Integrations>(
            'integrations'
        )) ?? (await store.get<Integrations>('intergrations'))) as
            | Integrations
            | undefined

        if (savedIntegrations) {
            joinServerForYouValue =
                savedIntegrations.roValra?.joinServerForYouValue ?? false
        }
    }
